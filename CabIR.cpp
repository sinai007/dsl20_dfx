...
boolean CabIR::begin(boolean status,float32_t gain, float32_t *fo, int nfor) 
{
	enabled = status;
	buffidx=0;
	k = 0;
...
void CabIR::impulse(const float32_t *guitar_cabinet_impulse, float32_t *maskgen,int size)
{
...
void CabIR::enable(boolean b) {
  enabled = b;
}
void CabIR::bypass(boolean b) {
  bypassed = b;
}

void CabIR::toggleEnable() {
  enabled = ! enabled;
}
void CabIR::toggleBypass() {
  bypassed = ! bypassed;
}


void CabIR::update(void) {
