...
{
public:
	CabIR(void) : AudioStream(2, inputQueueArray)                        
	{
    enabled = false;
    bypassed = false;
	}
  ...
  void bypass(boolean b);
  void toggleBypass();

  boolean isEnabled() { return enabled;}
  boolean isBypassed() { return bypassed;}
  
  boolean begin(boolean status, float32_t gain,float32_t *fo,int nfor);
...
private:
  ...
  boolean bypassed;
  int buffidx;
  const unsigned  partitionsize = 128;
